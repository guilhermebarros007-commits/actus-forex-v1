...
async def save_report(
    ...
    lux_decision: str,
    lux_raw: str,
    lux_decision_reason: str = "",
    risk_params: dict = None,
) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            """INSERT INTO reports
               (market_data, news, hype_analysis, oracle_analysis, vitalik_analysis, 
                lux_decision, lux_decision_reason, risk_params, lux_raw, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                ...
                lux_decision,
                lux_decision_reason,
                json.dumps(risk_params) if risk_params else "{}",
                lux_raw,
